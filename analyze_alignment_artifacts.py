...
def analyze_related_work():
    ...
    alignment_keywords = [
        'geometric', 'activation direction', 'safety', 'alignment', 
        'refusal', 'cohen', 'effect size', 'intervention', 'suppression'
    ]
    
    mechanistic_keywords = [
        'mechanistic', 'circuit', 'activation', 'representation',
    ...
    for row in cursor:
        ...
        alignment_score = sum(1 for keyword in alignment_keywords if keyword in full_text)
        mechanistic_score = sum(1 for keyword in mechanistic_keywords if keyword in full_text)
...
def print_analysis():
    ...
    general_related = [p for p in papers if p['total_relevance'] >= 2 and p not in high_alignment and p not in high_mechanistic]
    ...
    for section_name, section_papers in sections:
        if section_papers:
            ...
            for i, paper in enumerate(section_papers[:8], 1):  # Top 8 per section
                ...
                print(f"   🎯 Alignment: {paper['alignment_relevance']} | Mechanistic: {paper['mechanistic_relevance']}")
