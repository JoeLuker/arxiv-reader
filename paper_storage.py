...
class PaperStorage:
    ...
    def store_paper(self, paper: ArxivPaper, relevance_score: float = 0.0) -> bool:
        ...
        if logger.isEnabledFor(logging.DEBUG):
            ...
            assert (
                0 <= relevance_score <= 1
            ), f"Relevance score must be 0-1, got {relevance_score}"
        ...
        try:
            with self._get_connection() as conn:
                ...
                logger.debug(
                    f"Stored paper: {paper.id} with relevance {relevance_score:.3f}"
                )
                return True
    ...
    def get_papers(
        ...
        try:
            with self._get_connection() as conn:
                ...
                logger.info(
                    f"Retrieved {len(papers)} papers with relevance >= {min_relevance}"
                )
                return papers
    ...
    def star_paper(self, paper_id: str, starred: bool = True) -> bool:
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.execute(
                    "UPDATE papers SET is_starred = ? WHERE id = ?", (starred, paper_id)
                )
    ...
    def add_notes(self, paper_id: str, notes: str) -> bool:
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.execute(
                    "UPDATE papers SET notes = ? WHERE id = ?", (notes, paper_id)
                )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            with self._get_connection() as conn:
                ...
                # Starred papers
                cursor = conn.execute(
                    "SELECT COUNT(*) FROM papers WHERE is_starred = 1"
                )
                ...
                cursor = conn.execute(
                    "SELECT COUNT(*) FROM papers WHERE relevance_score > 0.7"
                )
