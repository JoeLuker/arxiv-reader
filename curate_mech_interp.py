...
def create_curated_timeline():
    ...
    for paper in papers:
        try:
            ...
            year = (
                int(paper["published"][:4])
                if paper["published"][:4].isdigit()
                else 2025
            )
        ...
        if (
            any(term in title_lower for term in ["grad-cam", "saliency", "gradient"])
            and year <= 2018
        ):
            ...
            any(
                term in title_lower
                for term in ["feature visualization", "activation", "lime", "shap"]
            )
            and year <= 2020
        ):
            ...
            any(
                term in title_lower + summary_lower
                for term in ["circuit", "attention heads", "mechanistic", "probe"]
            )
            and year <= 2023
        ):
        ...
        if section["papers"]:
            ...
            for i, paper in enumerate(section["papers"][:8], 1):  # Top 8 per section
                ...
                if "grad-cam" in title_lower:
                    print(
                        "   💡 Gradient-based visual explanations - foundational attribution method"
                    )
                    ...
                    print(
                        "   ⚡ Neural circuit analysis - understanding computation flow"
                    )
