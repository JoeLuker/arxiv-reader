...
class CitationAnalyzer:
    ...
    def extract_arxiv_references(self, paper: ArxivPaper) -> List[str]:
        ...
        logger.debug(
            f"Extracted {len(result)} arXiv references from paper {paper.id[:8]}"
        )
        return result
    ...
    def _log_network_stats(self, network: Dict[str, CitationData]):
        ...
        logger.info(
            f"  Most cited paper: {most_cited.paper_id[:8]} ({most_cited.citation_count} citations)"
        )
        logger.info(
            f"  Most references: {most_references.paper_id[:8]} ({most_references.reference_count} refs)"
        )

    def get_citation_score(
        self, paper_id: str, network: Dict[str, CitationData]
    ) -> float:
        """
        ...
        max_citations = max(
            (data.citation_count for data in network.values()), default=1
        )
        citation_score = (
            citation_data.citation_count / max_citations if max_citations > 0 else 0
        )
        ...
        if citation_data.references:
            ...
            if ref_citation_counts:
                ...
                reference_score = (
                    min(1.0, avg_ref_citations / max_citations)
                    if max_citations > 0
                    else 0
                )
        ...
        logger.debug(
            f"Citation score for {paper_id[:8]}: {final_score:.3f} "
    ...
    def find_influential_papers(
        self, network: Dict[str, CitationData], top_k: int = 10
    ) -> List[tuple]:
    ...
    def get_related_papers(
        self, paper_id: str, network: Dict[str, CitationData], max_distance: int = 2
    ) -> Set[str]:
        """
        ...
        logger.debug(
            f"Found {len(related)} related papers for {paper_id[:8]} "
            f"within distance {max_distance}"
        )

        return related

    def enhance_relevance_with_citations(
        self, papers: List[ArxivPaper], base_scores: List[float]
    ) -> List[float]:
        """
