...
def analyze_author_prominence():
    ...
    unknown_authors = {author for author, count in author_paper_count.items() if count <= 2}
    prominent_authors = {author for author, count in author_paper_count.items() if count >= 3}
...
def find_hidden_gems():
    ...
    for row in cursor:
        ...
        has_novelty_indicators = any(term in title_text for term in [
            'novel', 'new', 'towards', 'approach', 'method', 'framework'
        ])
        
        if (is_mostly_unknown or is_small_team) and has_novelty_indicators:
            paper['unknown_ratio'] = unknown_count / len(paper['authors']) if paper['authors'] else 0
