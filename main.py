        ...
        for paper in papers:
            try:
                ...
                if score >= config.MIN_RELEVANCE_SCORE:
                    ...
                    if success:
                        stored_count += 1
                        logger.debug(f"Stored paper: {paper.title[:50]}... (score: {score:.3f})")
                else:
                    logger.debug(f"Skipped low-relevance paper: {paper.title[:50]}... (score: {score:.3f})")
...
def print_papers(papers: List[dict], show_summary: bool = False):
    ...
    for i, paper in enumerate(papers, 1):
        ...
        print(f"   Relevance: {paper['relevance_score']:.3f} | Published: {paper['published'][:10]}")
...
def main():
    ...
    discover_parser.add_argument('--days', type=int, default=None, 
                               help=f'Days to look back (default: {config.DAYS_LOOKBACK})')
    
    # List command  
    list_parser = subparsers.add_parser('list', help='List stored papers')
    list_parser.add_argument('--limit', type=int, default=20, help='Number of papers to show')
    list_parser.add_argument('--min-relevance', type=float, default=None, 
                           help='Minimum relevance score')
    list_parser.add_argument('--summary', action='store_true', help='Show paper summaries')
    
    # Search command
    search_parser = subparsers.add_parser('search', help='Search for specific papers')
    search_parser.add_argument('query', help='Search query')
    search_parser.add_argument('--limit', type=int, default=20, help='Max results to fetch')
    ...
    mark_parser.add_argument('action', choices=['read', 'star', 'unstar'], help='Action to perform')
    ...
    try:
        ...
        if args.command == 'discover':
            ...
            papers = reader.list_papers(limit=args.limit, min_relevance=args.min_relevance)
