...
def find_rlhf_interpretability_papers():
    ...
    for row in cursor:
        ...
        has_interpretability = any(
            keyword in full_text for keyword in interpretability_keywords
        )

        if has_rlhf or (
            has_interpretability and ("human" in full_text or "feedback" in full_text)
        ):
...
def analyze_rlhf_papers():
    ...
    for paper in papers:
        ...
        elif "preference" in full_text and (
            "learning" in full_text or "model" in full_text
        ):
            preference_learning.append(paper)
        elif "reward" in full_text and "model" in full_text:
            reward_modeling.append(paper)
        elif any(
            term in full_text for term in ["alignment", "constitutional", "safety"]
        ):
    ...
    for paper in papers:
        ...
        if (
            has_training_analysis
            and has_rlhf_context
            and paper["relevance_score"] >= 0.16
        ):
    ...
    if promising:
        ...
        print(
            "This suggests a research gap in mechanistic interpretability of RLHF training!"
        )
