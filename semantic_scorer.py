...
class SemanticScorer:
    ...
    def _precompute_keyword_embeddings(self):
        ...
        try:
            ...
            logger.info(f"Computing embeddings for {len(keyword_texts)} keyword phrases")
            self.keyword_embeddings = self.model.encode(keyword_texts, convert_to_tensor=True)
            
            if logger.isEnabledFor(logging.DEBUG):
                assert self.keyword_embeddings is not None, "Keyword embeddings should not be None"
                assert len(self.keyword_embeddings) == len(keyword_texts), f"Embedding count mismatch"
    ...
    def score_paper_semantic(self, paper: ArxivPaper) -> float:
        ...
        if not self.model or self.keyword_embeddings is None:
            logger.warning("Model or keyword embeddings not available, falling back to 0.0")
            return 0.0
        ...
        try:
            ...
            similarities = cosine_similarity(
                paper_embedding.cpu().numpy(),
                self.keyword_embeddings.cpu().numpy()
            )[0]
            ...
            if logger.isEnabledFor(logging.DEBUG):
                assert 0 <= semantic_score <= 1, f"Semantic score must be 0-1, got {semantic_score}"
            
            logger.debug(f"Paper {paper.id[:8]} semantic score: {semantic_score:.3f} "
                        f"(max: {max_similarity:.3f}, mean_top: {mean_top_similarity:.3f})")
            
            return semantic_score
            
        except Exception as e:
            logger.warning(f"Failed to calculate semantic score for paper {paper.id}: {e}")
            return 0.0
    ...
    def score_papers_batch(self, papers: List[ArxivPaper]) -> List[float]:
        ...
        try:
            ...
            paper_embeddings = self.model.encode(paper_texts, convert_to_tensor=True, batch_size=32)
            
            # Calculate similarities with keyword embeddings
            similarities_matrix = cosine_similarity(
                paper_embeddings.cpu().numpy(),
                self.keyword_embeddings.cpu().numpy()
            )
            ...
            logger.info(f"Batch semantic scoring complete. Average score: {avg_score:.3f}")
    ...
    def get_most_similar_keywords(self, paper: ArxivPaper, top_k: int = 3) -> List[tuple]:
